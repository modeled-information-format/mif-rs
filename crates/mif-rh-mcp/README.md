...
tools, read-only over the index `mif-rh-cli review` builds. `suggest_type`
returns a ranked hypothesis and never writes to `reports/` — a human or agent
confirms it via rht's own `/ontology-review --enrich` step.
