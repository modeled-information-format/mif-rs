...
`attested-delivery/rust-template` repository. Workflows are organized by purpose and
...
| Pin Check | `attested-delivery/.github` » `pin-check.yml` | `pipeline.yml` | Asserts every `uses:` is pinned to a 40-char SHA |
| Sign and Attest Image | `attested-delivery/.github` » `sign-and-attest.yml` | `pipeline.yml` (push/tags) | Cosign signature, SLSA provenance, SBOM + vuln report as OCI referrers |
| Verify Image Attestations | `attested-delivery/.github` » `verify-attestation.yml` | `pipeline.yml` (push/tags) | Fail-closed verification gate before release |
...
- `docker-sign` — needs `docker` (push/tags only); calls the centralized
  `attested-delivery/.github` signer workflow, pinned by full commit SHA (SLSA
...
is renamed. They mirror the verified architecture of `attested-delivery/rlm-rs`.
...
**What it does:** Thin caller of the `attested-delivery/.github` central reusable
